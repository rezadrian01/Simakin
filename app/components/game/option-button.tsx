...
export function OptionButton({
  ...
  const variants: Record<OptionButtonProps["state"], string> = {
    idle: "border-2 border-muted-foreground/20 bg-card hover:border-simakin-primary/50 hover:bg-simakin-primary/5",
  ...
  return (
    <Button
      variant="outline"
      className={cn(
        "w-full text-left justify-start h-auto py-3 px-4 font-normal text-base transition-all",
        variants[state]
      )}
      onClick={onClick}
      disabled={disabled}
