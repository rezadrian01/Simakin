...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const questions = await generateQuestions("LENGKAPI_AYAT", QUESTIONS_PER_ROUND);
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const gameSession = await db.gameSession.create({
    data: {
      userId,
      gameType: GameType.LENGKAPI_AYAT,
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold">Lengkapi Ayat</h1>
          <p className="text-sm text-muted-foreground">
            Pilih kata yang tepat untuk mengisi kosong
          </p>
        </div>
      </div>
          ...
          >
            <div className="mb-4">
              <p className="text-xs text-muted-foreground mb-2 text-center">
                {currentQuestion.surahName} ayat {currentQuestion.ayahNumber}
              </p>
              <p
                className="text-2xl font-serif text-right leading-relaxed text-foreground bg-muted/30 rounded-lg p-4"
                dir="rtl"
              >
                {currentQuestion.ayahWithBlank}
              </p>
            </div>

            <p className="text-sm text-muted-foreground text-center mb-4">
              Pilih kata yang mengisi "___":
            </p>
